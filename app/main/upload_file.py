...
class uploadfile():
    def __init__(self, name, type=None, size=None, not_allowed_msg=''):
        ...
        if self.type and self.type.startswith('video'):
            self.thumbnail_url = "thumbnail/%s.jpg" % (name[:name.rindex('.')])
        else:
    ...
    def is_image(self):
        fileName, fileExtension = splitext(self.name.lower())
    ...
    def get_file(self):
        if self.type != None:
            # POST an image
            if self.type.startswith('image') or self.type.startswith('video'):
